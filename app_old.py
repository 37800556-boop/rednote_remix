...
<style>
    ...
    .floating-config:hover {
        transform: scale(1.1);
        box-shadow: 0 6px 25px rgba(102,126,234,0.5);
    }

    /* 配置按钮提示 */
    .floating-config::before {
        content: "配置";
        position: absolute;
        top: -35px;
        left: 50%;
    ...
    .floating-config:hover::before {
        opacity: 1;
        visibility: visible;
    }

    /* 隐藏 Streamlit 默认的菜单按钮 */
    button[kind="header"] {
        display: none !important;
    }
</style>

<button class="floating-config" onclick="document.querySelectorAll('button[aria-label="点击打开配置"]')[0]?.click()">⚙</button>
""", unsafe_allow_html=True)
...
# 配置按钮（隐藏，供HTML按钮触发）
if st.button("⚙️ 配置", key="config_trigger", help="点击打开配置"):
    pass

# 配置弹窗
with st.expander("⚙️ API 配置", expanded=False, key="config_expander"):
    st.markdown("### 🔑 API 配置")
    ...
    with st.container():
        ...
        <div class="config-dialog-overlay" onclick="closeConfig()"></div>
        """, unsafe_allow_html=True)
        ...
        with st.container():
            ...
            with col_close2:
                if st.button("关闭", use_container_width=True, key="close_config_btn"):
